<template>
  <div v-if="hotelData">
    <q-carousel swipeable animated v-model="slide" thumbnails infinite class="bg-grey-10">
      <q-carousel-slide
    ...
    <q-page padding>
      <div class="text-h4 text-accent">{{hotelData.name}}</div>
      <q-badge
        class="q-mr-xs"
        v-for="(amenity, index) in hotelData.amenities"
        :key="index"
        color="accent"
      >{{amenity.name}}</q-badge>
      <div class="row min-height q-pt-sm">
        <div class="col-8">
          ...
          <div v-for="(review, key) in hotelData.guest_reviews" :key="key">
            <div class="text-h6 text-accent">{{review.title}}</div>
        ...
        <div class="map-view col-4">
          <MapView :lat="hotelData.lat" :lng="hotelData.lng" :hotels="[hotelData]"/>
        </div>
      </div>
    </q-page>
    ...
    <q-footer bordered class="bg-white text-black">
      <q-toolbar>
        ...
        <q-btn
          v-if="roundTo(hotelData.nightly_rate)"
          flat
          class="bg-accent text-white"
        >Book hotel for {{roundTo(hotelData.nightly_rate)}}</q-btn>
        <q-btn v-else flat class="bg-accent text-white">Not Available</q-btn>
      </q-toolbar>
    </q-footer>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    await this.$axios
      .get(
        `https://roomkey-api.herokuapp.com/api/locations/${this.search
          .toLowerCase()
          .replace(/\s/g, "")}/hotels/${this.hotelId}?checkin=${
          this.arives
        }&checkout=${this.departs}`
      )
      .then(response => {
